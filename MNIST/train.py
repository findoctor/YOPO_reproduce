...
optimizer = optim.SGD(net.other_layers.parameters(), lr =1e-2 / 5, momentum=0.9, weight_decay=5e-4 ) 
first_layer_optimizer = optim.SGD(net.layer_one.parameters(), lr = 1e-2 / 5, momentum=0.9, weight_decay=5e-4)
# prepare dataset
...
def visualize(time_arr, yopo_clean_err, yopo_robust_err):
    ...
    ax1.plot(time_arr, yopo_clean_err, 'g', label='YOPO-5-10 Clean Error')
    ax1.plot(time_arr, yopo_robust_err, color = 'g', linestyle='-.', label='YOPO-5-10 Robust Error')
